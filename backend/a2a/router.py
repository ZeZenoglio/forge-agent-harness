...
async def get_agent_card() -> JSONResponse:
    ...
    card = {
        ...
        "endpoints": {"tasks_send": "/a2a/tasks/send", "tasks_get": "/a2a/tasks/get"},
    }
...
async def tasks_send(
    ...
    return A2ATaskResponse(
        id=req.id, result={"task_id": session_id, "status": "working"}
    )
...
async def tasks_get(req: A2ATaskRequest) -> A2ATaskResponse:
    ...
    return A2ATaskResponse(
        id=req.id, result={"task_id": task_id, "status": "completed"}
    )
...
async def tasks_stream(task_id: str) -> StreamingResponse:
    return StreamingResponse(
        stream_service.subscribe(task_id), media_type="text/event-stream"
    )
