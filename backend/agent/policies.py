...
class PolicyEngine:
    ...
    def __init__(
        ...
        self.secret_patterns = [
            re.compile(
                r"(?i)(?:api_key|secret|token|password|auth|bearer)[^a-z0-9]*[a-z0-9_]{16,}"
            ),
        ...
        if tool_name == "execute_shell":
            ...
            if "sudo" in command:
                raise ApprovalRequiredException(
                    f"Command requires human approval: {command}"
                )

        # We can expand this list (e.g., deleting files, sending emails)
        if tool_name == "delete_file":
            raise ApprovalRequiredException(
                f"Deleting file requires human approval: {arguments.get('path')}"
            )

        if tool_name == "send_email":
            raise ApprovalRequiredException(
                f"Sending email to {arguments.get('to_address')} requires human approval."
            )
        ...
        for quote in significant_quotes:
            if quote not in combined_context:
                ...
                if norm_quote not in norm_context:
                    raise ValueError(
                        f"Hallucinated citation detected. Quote not found in context: '{quote}'"
                    )
        return True
